...
class ACLMiddleware(object):
    ...
    def process_view(self, request, view_func, *view_args, **view_kwargs):
        if (hasattr(view_func, 'required_privilege')):
            ...
            if callable(privilege['resource']):
                resource = privilege['resource'](request)
            else:
                resource = privilege['resource']
            granted = is_allowed(request, privilege['operation'], resource)
            if granted:
                return None
            ...
            return self._create_403_response(request, privilege['operation'], resource,
